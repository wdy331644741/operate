...
class Captcha
{
     ...
     * @param $sessionHandle
     */
    public function __construct($sessionHandle, $config = [])
    {
    ...
    private function check($code, $id = '')
    {
        ...
        // 验证码不能为空
        $secode = $this->sessionHandle->get($key);
...
//            $this->reset && Session::delete($key);  //不重置
            return true;
        }
    ...
    public function entry()
    {
        ...
        //验证码数据
        $captchaData = $this->sessionHandle->get(self::IMG_CAPTCHA_FLAG);

        $code = $captchaData['code']; // 验证码
